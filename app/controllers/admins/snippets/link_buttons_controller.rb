class Admins::Snippets::LinkButtonsController < Admins::BaseController
	before_action :set_snippet
	before_action :set_link_button, except: [:new, :create]

  def new
		@link_button = @snippet.link_buttons.build
  ...
  def create
		@success = false
		@link_button = @snippet.link_buttons.build(params_link_button)
    if @link_button.save
  ...
  def set_link_button
		@link_button = @snippet.link_buttons.find(params[:id])
		set_objectables
  end
